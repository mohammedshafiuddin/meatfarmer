...
exports.getHospitalPatientHistory = exports.getHospitalTokenHistory = exports.searchToken = exports.createLocalToken = exports.createOfflineToken = exports.getDoctorTodaysTokens = exports.updateTokenStatus = exports.getHospitalTodaysTokens = exports.getDoctorAvailabilityForNextDays = exports.updateDoctorAvailability = exports.bookToken = void 0;
...
const roles_manager_1 = require("../lib/roles-manager");
/**
...
const bookToken = async (req, res, next) => {
    try {
        ...
        if (!doctorId || !userId || !tokenDate) {
            throw new api_error_1.ApiError("Missing required fields: doctorId, userId, and tokenDate are required", 400);
        }
        ...
        const doctor = await db_index_1.db.query.usersTable.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.usersTable.id, doctorId),
        });
        if (!doctor) {
            throw new api_error_1.ApiError("Doctor not found", 404);
        }
        ...
        const doctorInfo = await db_index_1.db.query.doctorInfoTable.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.doctorInfoTable.userId, doctorId),
        });
        if (!doctorInfo) {
            throw new api_error_1.ApiError("User is not registered as a doctor", 400);
        }
        ...
        const availability = await db_index_1.db.query.doctorAvailabilityTable.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.doctorId, doctorId), (0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.date, formattedDate)),
        });
        // If no availability record exists, doctor is not available for booking
        if (!availability) {
            throw new api_error_1.ApiError("Doctor is not available for booking on this date", 400);
        }
        // If doctor has stopped accepting tokens for that day
        if (availability.isStopped) {
            throw new api_error_1.ApiError("Doctor is not accepting appointments for this date", 400);
        }
        ...
        if (availableTokens <= 0) {
            throw new api_error_1.ApiError("No more appointments available for this date", 400);
        }
        ...
        return await db_index_1.db.transaction(async (tx) => {
            ...
            const [newToken] = await tx
                .insert(schema_1.tokenInfoTable)
                .values({
                doctorId: doctorId,
                userId: userId,
            ...
            await tx
                .update(schema_1.doctorAvailabilityTable)
                .set({
                filledTokenCount: filledTokens + 1,
            })
                .where((0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.id, availability.id));
...
const updateDoctorAvailability = async (req, res, next) => {
    try {
        ...
        if (!doctorId || !date || tokenCount === undefined) {
            throw new api_error_1.ApiError("Missing required fields: doctorId, date, and tokenCount are required", 400);
        }
        // Parse the date in a timezone-aware manner to avoid date shifting
        // Expect date in YYYY-MM-DD format from the frontend
        let formattedDate;
        ...
        const existingAvailability = await db_index_1.db.query.doctorAvailabilityTable.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.doctorId, doctorId), (0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.date, formattedDate)),
        });
        // Use a transaction to ensure data consistency
        return await db_index_1.db.transaction(async (tx) => {
            ...
            if (existingAvailability) {
                ...
                if (tokenCount < existingAvailability.filledTokenCount) {
                    throw new api_error_1.ApiError(`Cannot reduce token count below already filled tokens (${existingAvailability.filledTokenCount})`, 400);
                }
                ...
                [availability] = await tx
                    ...
                    filledTokenCount: filledTokenCount !== undefined
                        ? filledTokenCount
                        : existingAvailability.filledTokenCount,
                    consultationsDone: consultationsDone !== undefined
                        ? consultationsDone
                        : existingAvailability.consultationsDone,
                    isLeave: isLeave !== undefined ? isLeave : existingAvailability.isLeave,
                })
                    .where((0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.id, existingAvailability.id))
                    .returning();
            }
            else {
                // Create new availability record
                [availability] = await tx
                    .insert(schema_1.doctorAvailabilityTable)
                    .values({
                    doctorId: doctorId,
                    date: formattedDate,
                    totalTokenCount: tokenCount,
 ...
 * @param next Next function
 */
const getDoctorAvailabilityForNextDays = async (req, res, next) => {
    try {
        ...
        // Validate doctor ID
        if (!doctorId) {
            throw new api_error_1.ApiError("Missing required query parameter: doctorId", 400);
        }
        // Check if doctor exists and is a doctor
        const doctor = await db_index_1.db.query.usersTable.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.usersTable.id, parseInt(doctorId)),
        });
        if (!doctor) {
            throw new api_error_1.ApiError("Doctor not found", 404);
        }
        const doctorInfo = await db_index_1.db.query.doctorInfoTable.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.doctorInfoTable.userId, parseInt(doctorId)),
        });
        if (!doctorInfo) {
            throw new api_error_1.ApiError("User is not registered as a doctor", 400);
        }
        ...
        const availabilityResults = await Promise.all(nextDays.map(async (date) => {
            const availability = await db_index_1.db.query.doctorAvailabilityTable.findFirst({
                where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.doctorId, parseInt(doctorId)), (0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.date, date)),
            });
            return {
                date,
...
exports.getDoctorAvailabilityForNextDays = getDoctorAvailabilityForNextDays;
/**
 ...
 * @param next Next function
 */
const getHospitalTodaysTokens = async (req, res, next) => {
    try {
        ...
        const hospitalEmployee = await db_index_1.db.query.hospitalEmployeesTable.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.userId, userId),
        });
        if (!hospitalEmployee) {
            throw new api_error_1.ApiError("User is not associated with any hospital", 404);
        }
        const hospitalId = hospitalEmployee.hospitalId;
        // Get the hospital name
        const hospital = await db_index_1.db.query.hospitalTable.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.hospitalTable.id, hospitalId),
        });
        if (!hospital) {
            throw new api_error_1.ApiError("Hospital not found", 404);
        }
        // Get all doctors who work at this hospital
        const hospitalDoctors = await db_index_1.db.query.hospitalEmployeesTable.findMany({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.hospitalId, hospitalId), (0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.designation, const_strings_1.DESIGNATIONS.DOCTOR)),
        });
        // Extract doctor IDs
        ...
        for (const doctorId of doctorIds) {
            ...
            const doctorSpecializations = await db_index_1.db.query.doctorSpecializationsTable.findMany({
                where: (0, drizzle_orm_1.eq)(schema_1.doctorSpecializationsTable.doctorId, doctorId),
                with: {
                    specialization: true,
                },
            });
            ...
            const availability = await db_index_1.db.query.doctorAvailabilityTable.findFirst({
                where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.doctorId, doctorId), (0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.date, today)),
            });
            // Get tokens for today with patient details
            const tokens = await db_index_1.db.query.tokenInfoTable.findMany({
                where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.tokenInfoTable.doctorId, doctorId), (0, drizzle_orm_1.eq)(schema_1.tokenInfoTable.tokenDate, today)),
                with: {
                    user: {
                        with: {
                            mobileNumber: true,
                ...
                orderBy: schema_1.tokenInfoTable.queueNum,
            });
...
exports.getHospitalTodaysTokens = getHospitalTodaysTokens;
/**
 * Update token status
 *
...
const updateTokenStatus = async (req, res, next) => {
    try {
        ...
        // Validate token ID
        if (isNaN(tokenId)) {
            throw new api_error_1.ApiError("Invalid token ID", 400);
        }
        // Validate required fields
        if (!status && !consultationNotes) {
            throw new api_error_1.ApiError("Status or consultation notes are required", 400);
        }
        ...
        if (status && !validStatuses.includes(status)) {
            throw new api_error_1.ApiError("Invalid status. Must be one of: UPCOMING, IN_PROGRESS, COMPLETED, MISSED, CANCELLED", 400);
        }
        // Check if token exists
        const token = await db_index_1.db.query.tokenInfoTable.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.tokenInfoTable.id, tokenId),
        });
        if (!token) {
            throw new api_error_1.ApiError("Token not found", 404);
        }
        ...
        if (!isDoctor) {
            ...
            const doctorInSameHospital = await db_index_1.db.query.hospitalEmployeesTable.findFirst({
                where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.userId, token.doctorId)),
            });
            if (doctorInSameHospital) {
                const userHospital = await db_index_1.db.query.hospitalEmployeesTable.findFirst({
                    where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.userId, userId), (0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.hospitalId, doctorInSameHospital.hospitalId)),
                });
                isHospitalAdmin = !!userHospital;
            }
        }
        if (!isDoctor && !isHospitalAdmin) {
            throw new api_error_1.ApiError("Not authorized to update this token", 403);
        }
        // Update token status
        const updateData = {};
        if (status)
        ...
        const [updatedToken] = await db_index_1.db
            .update(schema_1.tokenInfoTable)
            .set(updateData)
            .where((0, drizzle_orm_1.eq)(schema_1.tokenInfoTable.id, tokenId))
            .returning();
        ...
        if (status === "COMPLETED" || status === "MISSED") {
            ...
            const availability = await db_index_1.db.query.doctorAvailabilityTable.findFirst({
                where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.doctorId, token.doctorId), (0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.date, token.tokenDate)),
            });
            if (availability) {
 ...
 * @param next Next function
 */
const getDoctorTodaysTokens = async (req, res, next) => {
    try {
        const doctorId = parseInt(req.params.doctorId);
        if (isNaN(doctorId)) {
            throw new api_error_1.ApiError("Invalid doctor ID", 400);
        }
        ...
        if (!isAuthorized) {
            ...
            if (userHospital) {
                const doctorInSameHospital = await db_index_1.db.query.hospitalEmployeesTable.findFirst({
                    where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.userId, doctorId), (0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.hospitalId, userHospital.hospitalId)),
                });
                isAuthorized = !!doctorInSameHospital;
            }
        }
        if (!isAuthorized) {
            throw new api_error_1.ApiError("Not authorized to view this doctor's tokens", 403);
        }
        ...
        const onlineTokens = await db_index_1.db.query.tokenInfoTable.findMany({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.tokenInfoTable.doctorId, doctorId), (0, drizzle_orm_1.eq)(schema_1.tokenInfoTable.tokenDate, today)),
            with: {
                user: {
                    with: {
...
exports.getDoctorTodaysTokens = getDoctorTodaysTokens;
/**
 ...
 * @param next Next function
 */
const createOfflineToken = async (req, res, next) => {
    try {
        ...
        if (!doctorId || !patientName || !mobileNumber || !tokenDate) {
            throw new api_error_1.ApiError("Missing required fields: doctorId, patientName, mobileNumber, and tokenDate are required", 400);
        }
        if (!adminUserId) {
            throw new api_error_1.ApiError("User not authenticated", 401);
        }
        ...
        const doctorHospitalEmployee = await db_index_1.db.query.hospitalEmployeesTable.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.userId, doctorId),
        });
        if (!doctorHospitalEmployee) {
            throw new api_error_1.ApiError("Doctor is not associated with any hospital", 404);
        }
        const adminHospitalEmployee = await db_index_1.db.query.hospitalEmployeesTable.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.userId, adminUserId), (0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.hospitalId, doctorHospitalEmployee.hospitalId), (0, drizzle_orm_1.eq)(schema_1.hospitalEmployeesTable.designation, const_strings_1.DESIGNATIONS.HOSPITAL_ADMIN) // Ensure admin is a hospital admin
            ),
        });
        if (!adminHospitalEmployee) {
            throw new api_error_1.ApiError("Not authorized to create offline tokens for this doctor. Only hospital admins can create offline tokens for doctors in their hospital.", 403);
        }
        ...
        return await db_index_1.db.transaction(async (tx) => {
            ...
            const [newOfflineToken] = await tx
                .insert(schema_1.offlineTokensTable)
                .values({
                doctorId: doctorId,
...
const createLocalToken = async (req, res, next) => {
    const { mobileNumber, patientName, age, gender, reason, doctorId } = req.body;
    // --- Validation ---
    if (!mobileNumber || !patientName || !doctorId) {
        throw new api_error_1.ApiError("Missing required fields: mobileNumber, patientName, and doctorId are required", 400);
    }
    const tokenDate = new Date().toISOString().split("T")[0]; // Token for today
    // --- Main Logic in Transaction ---
    const result = await db_index_1.db.transaction(async (tx) => {
        // 1. Find or create user
        let mobileRecord = await tx.query.mobileNumbersTable.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.mobileNumbersTable.mobile, mobileNumber),
        });
        if (!mobileRecord) {
        ...
        let user = await tx.query.usersTable.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.usersTable.mobileId, mobileRecord.id), (0, drizzle_orm_1.eq)(schema_1.usersTable.name, patientName)),
        });
        if (!user) {
            [user] = await tx
            ...
            const generalUserRole = await tx.query.roleInfoTable.findFirst({
                where: (0, drizzle_orm_1.eq)(schema_1.roleInfoTable.name, roles_manager_1.ROLE_NAMES.GENERAL_USER)
            });
            if (generalUserRole) {
                await tx
        ...
        const availability = await tx.query.doctorAvailabilityTable.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.doctorId, doctorId), (0, drizzle_orm_1.eq)(schema_1.doctorAvailabilityTable.date, tokenDate)),
        });
        if (!availability) {
            throw new api_error_1.ApiError("Doctor is not available for booking on this date", 400);
        }
...
const searchToken = async (req, res, next) => {
    ...
    const today = new Date().toISOString().split("T")[0];
    const conditions = [
        (0, drizzle_orm_1.eq)(schema_1.tokenInfoTable.doctorId, Number(doctorId)),
        (0, drizzle_orm_1.eq)(schema_1.tokenInfoTable.tokenDate, today),
    ];
    if (query) {
        const searchQuery = `%${String(query).toLowerCase()}%`;
        conditions.push((0, drizzle_orm_1.or)((0, drizzle_orm_1.like)((0, drizzle_orm_1.sql) `CAST(${schema_1.tokenInfoTable.queueNum} AS TEXT)`, searchQuery), (0, drizzle_orm_1.like)((0, drizzle_orm_1.sql) `LOWER(${schema_1.usersTable.name})`, searchQuery)));
    }
    const onlineTokens = await db_index_1.db.select({
        id: schema_1.tokenInfoTable.id,
        patientId: schema_1.usersTable.id,
        queueNum: schema_1.tokenInfoTable.queueNum,
        patientName: schema_1.usersTable.name,
        patientMobile: schema_1.mobileNumbersTable.mobile,
        description: schema_1.tokenInfoTable.description,
        status: schema_1.tokenInfoTable.status,
    }).from(schema_1.tokenInfoTable)
        .leftJoin(schema_1.usersTable, (0, drizzle_orm_1.eq)(schema_1.usersTable.id, schema_1.tokenInfoTable.userId))
 ...
 * @param next Next function
 */
const getHospitalTokenHistory = async (req, res, next) => {
    const userId = req.user?.userId;
    if (!userId) {
        throw new api_error_1.ApiError("User not authenticated", 401);
    }
    ...
    if (!hospitalEmployee ||
        hospitalEmployee.designation !== const_strings_1.DESIGNATIONS.HOSPITAL_ADMIN) {
        throw new api_error_1.ApiError("Not authorized to view token history", 403);
    }
    const hospitalId = hospitalEmployee.hospitalId;
    ...
    // --- Filter Logic ---
    const { doctorIds: doctorIdsQuery, patientIds: patientIdsQuery, statuses: statusesQuery, startDate, endDate, } = req.query;
    const conditions = [(0, drizzle_orm_1.inArray)(schema_1.tokenInfoTable.doctorId, hospitalDoctorIds)];
    if (doctorIdsQuery) {
        const filteredDoctorIds = doctorIdsQuery
            .split(",")
            .map(Number);
        if (filteredDoctorIds.length > 0) {
            conditions.push((0, drizzle_orm_1.inArray)(schema_1.tokenInfoTable.doctorId, filteredDoctorIds));
        }
    }
    if (patientIdsQuery) {
        const filteredPatientIds = patientIdsQuery
            .split(",")
            .map(Number);
        if (filteredPatientIds.length > 0) {
            conditions.push((0, drizzle_orm_1.inArray)(schema_1.tokenInfoTable.userId, filteredPatientIds));
        }
    }
    if (statusesQuery) {
        const filteredStatuses = statusesQuery.split(",");
        if (filteredStatuses.length > 0) {
            conditions.push((0, drizzle_orm_1.inArray)(schema_1.tokenInfoTable.status, filteredStatuses));
        }
    }
    if (startDate) {
        conditions.push((0, drizzle_orm_1.gte)(schema_1.tokenInfoTable.tokenDate, startDate));
    }
    if (endDate) {
        conditions.push((0, drizzle_orm_1.lte)(schema_1.tokenInfoTable.tokenDate, endDate));
    }
    // --- Pagination ---
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const offset = (page - 1) * limit;
    const combinedConditions = (0, drizzle_orm_1.and)(...conditions);
    // --- Database Query ---
    const tokens = await db_index_1.db.query.tokenInfoTable.findMany({
        ...
        orderBy: [(0, drizzle_orm_1.desc)(schema_1.tokenInfoTable.tokenDate), schema_1.tokenInfoTable.queueNum],
        limit: limit,
        offset: offset,
    });
    // Get total count with the same filters for pagination
    const totalCountResult = await db_index_1.db
...
exports.getHospitalTokenHistory = getHospitalTokenHistory;
/**
...
const getHospitalPatientHistory = async (req, res, next) => {
    const userId = req.user?.userId;
    if (!userId) {
        throw new api_error_1.ApiError("User not authenticated", 401);
    }
    ...
    if (!hospitalEmployee ||
        hospitalEmployee.designation !== const_strings_1.DESIGNATIONS.HOSPITAL_ADMIN) {
        throw new api_error_1.ApiError("Not authorized to view patient history", 403);
    }
    ...
    // --- Filter Logic ---
    const { patientIds: patientIdsQuery, doctorIds: doctorIdsQuery, statuses: statusesQuery, startDate, endDate, } = req.query;
    const conditions = [(0, drizzle_orm_1.inArray)(schema_1.tokenInfoTable.doctorId, hospitalDoctorIds)];
    if (patientIdsQuery) {
        const filteredPatientIds = patientIdsQuery
            .split(",")
    ...
    const allPatients = Object.entries(tokensByPatient).map(([userId, userTokens]) => {
        ...
        const sortedTokens = [...userTokens].sort((a, b) => new Date(a.tokenDate).getTime() - new Date(b.tokenDate).getTime());
        ...
        return {
            ...
            age: userTokens[0].user.age || 0, // Cast to any for now
            gender: userTokens[0].user.gender || '', // Cast to any for now
            totalTokens,
            completedTokens,
            upcomingTokens,
            firstVisitDate,
...
exports.getHospitalPatientHistory = getHospitalPatientHistory;
