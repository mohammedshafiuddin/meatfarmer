...
exports.seed = seed;
const db_index_js_1 = require("./db_index.js");
...
async function seed() {
    try {
        ...
        const existingRoles = await db_index_js_1.db.select().from(schema_js_1.roleInfoTable);
        ...
        if (newRoles.length > 0) {
            ...
            insertedRoles = await db_index_js_1.db.insert(schema_js_1.roleInfoTable)
                .values(newRoles)
                .returning({ id: schema_js_1.roleInfoTable.id, name: schema_js_1.roleInfoTable.name });
        }
        else {
            console.log('All roles already exist. No new roles added.');
        }
        // Get all roles (existing + newly inserted)
        const allRoles = await db_index_js_1.db.select().from(schema_js_1.roleInfoTable);
        ...
        const existingSpecializations = await db_index_js_1.db.select().from(schema_js_1.specializationsTable);
        ...
        if (newSpecializations.length > 0) {
            ...
            await db_index_js_1.db.insert(schema_js_1.specializationsTable).values(newSpecializations);
